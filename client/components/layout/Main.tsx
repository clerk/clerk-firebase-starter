...
export function Main({ children }: MainProps) {
  return (
    <>
      <Flex as="nav" padding={4} bg="brand.blue" color="white" justifyContent="space-between" alignItems="center">
...
const SignOutButton = () => {
  ...
  const signOutAll = async () => await Promise.all([signOut(), firebase.auth().signOut()]);
...
const SignInButton = () => {
  ...
  return (
    <Button _hover={{ bg: "gray.600" }} variant="ghost" onClick={() => openSignIn({})}>
      Sign in
    </Button>
  );
};
